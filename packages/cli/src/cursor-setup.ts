...
2. Call \`get_workflow\` with the user's goal and a project fingerprint. It returns the same decision as \`build_context\` and \`build_plan\`, plus \`documentationTargets\`, \`debugTargets\`, \`evalTargets\`, and \`observabilityTargets\`. It does not call Context7, Promptfoo, or Langfuse.
3. Read \`documentationTargets\`. For each target, query Context7 for that library, the target reason, and the detected version. Treat Context7 as the source of truth for current library syntax.
4. Treat PromptMarket as the source of truth for architecture, the AI engineering pattern, implementation sequence, prompting strategy, and which checks matter.
5. If current documentation conflicts with a PromptMarket guide, follow the current documentation, adapt the guide, and mention the discrepancy.
6. Implement using the existing repository's conventions. Do not paste instrumentation from memory.
7. When behavior is wrong, read \`debugTargets\` and follow the diagnosis order. For \`ai-sdk-devtools\`, follow current AI SDK docs to register DevTools for the installed version, then run \`npx @ai-sdk/devtools@latest\`. DevTools stores prompts and tool data locally in plain text. Use it only in local development.
8. Before considering the work complete, scaffold and run the Promptfoo checks in \`evalTargets\` with \`promptmarket verify init\`, then \`promptmarket verify run\`. Promptfoo owns the evaluator.
9. Before merge, run \`promptmarket verify ci --github\` so the Promptfoo GitHub Action gates the pull request. Promptfoo posts the result and fails the job when the suite fails.
10. When production tracing is in scope, read \`observabilityTargets\`. For Langfuse, query current Langfuse and AI SDK docs, inspect any existing OpenTelemetry setup, and integrate without replacing an existing tracer. \`promptmarket observe setup langfuse\` prints packages, environment names, and metadata. It does not edit application code.
