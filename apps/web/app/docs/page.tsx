...
pnpm dlx @promptmarket/cli guide ai-product-brief-builder`}
        ...
        />
        <p className="note">
          <code>feature init --write</code> stores a version-controlled contract for one AI feature. <code>--path</code> records the files that feature owns. <code>feature attach</code> adds paths to an existing contract. <code>feature adopt</code> records a feature that already exists and does not edit its implementation. <code>impacted --base</code> lists features affected since that git ref, using the same three-dot comparison GitHub uses for a pull request, plus uncommitted files. Owned paths, the feature contract, and its eval suite all count. A package manifest or lockfile counts only when a package that contract tested with changed version. <code>feature review --base</code> lists contract checks for those features. <code>verify changed --base</code> runs only their Promptfoo suites. <code>feature status</code> checks a contract. <code>detect</code> fingerprints the current project from{" "}
