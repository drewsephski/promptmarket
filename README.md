...
The CLI reads the latest catalog from `https://promptmarket.sh/api/content/v1`. A cache younger than 15 minutes is used immediately. After that, the CLI revalidates with an ETag. If the network fails, it uses the stale cache, then the bundled snapshot. `--refresh` skips the freshness window. `--offline` skips the network. `setup cursor --write` merges the hosted MCP server into `.cursor/mcp.json` and adds an Apply Intelligently rule. `--with-context7` hands off to Context7's official Cursor setup when that server is not already configured. `research` is optional: it reconciles a plan with live docs through the Context7 SDK and needs `CONTEXT7_API_KEY`. `plan` does not. `feature init --write` records a small contract in `.promptmarket/features/` and a Promptfoo suite beside it. The contract stores identities and compatibility references, not the generated plan. `feature status` reports that contract. `feature check --all` fails only on objective problems. `verify init` writes a thin Promptfoo suite from the plan's eval target. `verify run` and `verify view` delegate to Promptfoo. `verify ci --github` writes a Promptfoo GitHub Action that runs `.promptmarket/evals` on pull requests and fails the job when the suite fails. `observe` names a Langfuse production target when the plan has one. `observe setup langfuse` prints packages and environment names and does not edit application code. `detect` reads `package.json`, the lockfile, and a few config filenames in the project directory. It does not read source files or `.env`.
